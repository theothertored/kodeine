...
export class KodeFunctionWithModes extends IKodeFunction {
        ...
        switch (argPatternElements.type) {
            ...
            case 'num':
                ...
                if (argValue.isNumeric) {
                    ...
                    throw new InvalidArgumentError(`${call.func.getName()}(${modeName})`, argPatternElements.name, i, call.args[i], argValue, `Argument must be numeric.`);
                }
            default:
    ...
    call(evalCtx, call, args) {
        ...
        if (!mode) {
            ...
            throw new InvalidArgumentError(`${call.func.getName()}()`, 'mode', 0, call.args[0], args[0], `Mode "${modeName}" not found.`);
        }
            ...
            if (!argPatternMatch) {
                ...
                if (argPatternElements.isOptional) {
                    ...
                    if (optionalArgumentEncountered) {
                        ...
                        throw new InvalidArgumentCountError(call, `Argument #${i + 1} (${argPatternElements.name}) missing.`, `${call.func.getName()}(${modeName})`);
                    }
                }
                ...
                if (argPatternElements.isRest) {
                    ...
                    if (argPatternElements.restMinCount && remainingParamCount < argPatternElements.restMinCount) {
                        ...
                        throw new InvalidArgumentCountError(call, `At least ${argPatternElements.restMinCount} argument${argPatternElements.restMinCount === 1 ? '' : 's'} required.`, `${call.func.getName()}(${modeName})`);
                    }
                    else {

...
import { daysIntoYear, padWithZeros } from "../helpers/utils.js";
...
type SimpleTokenImplementations = Record<string, ((date: Date) => string | number) | undefined>;
type MultiTokenImplementations = Record<string, ((date: Date, match: string) => string | number) | undefined>;
 ...
 */
function format(date: Date, format: string, simpleTokens: SimpleTokenImplementations, multiTokens: MultiTokenImplementations): string {
...
export class DfFunction extends IKodeFunction {
    ...
    call(evalCtx: EvaluationContext, call: FunctionCall, args: KodeValue[]): KodeValue {
         ...
         */
        const simpleTokens: SimpleTokenImplementations = {
         ...
         */
        const multiTokens: MultiTokenImplementations = {

            // hour (0-23 regardless of mode)
            'H': (date, match) => padWithZeros(date.getHours(), match.length),
            ...
            'h': (date, match) => {
                ...
                if (resolveClockMode() == '12h')
                    ...
                    return padWithZeros(date.getHours() % 12 === 0 ? 12 : date.getHours() % 12, match.length);
                else
                    // getHours() returns 0-23, which is good
                    return padWithZeros(date.getHours(), match.length);

            },

            // minute
            'm': (date, match) => padWithZeros(date.getMinutes(), match.length),

            // second
            's': (date, match) => padWithZeros(date.getSeconds(), match.length),
            ...
            'k': (date, match) => {
                ...
                if (resolveClockMode() == '12h')
                    ...
                    return padWithZeros(date.getHours() % 12, match.length);
                else
                    // getHours() returns 0-23, replace 0 with 24, keep 1-23 as is
                    return padWithZeros(date.getHours() === 0 ? 24 : date.getHours(), match.length);

            },

            // day of month
            'd': (date, match) => padWithZeros(date.getDate(), match.length),

            // day of year
            'D': (date, match) => padWithZeros(daysIntoYear(date), match.length),
            ...
            'M': (date, match) => {
                ...
                if (match.length < 3)
                    ...
                    return padWithZeros(date.getMonth() + 1, match.length);
            ...
            // year
            'y': (date, match) => match.length == 2 ? date.getFullYear().toString().substring(2) : padWithZeros(date.getFullYear(), match.length),
        ...
        if (args.length === 1)
            ...
            dateToFormat = KustomDateHelper.parseKustomDateString(evalCtx.getNow(), args[1].text);

        // format and print the date
        return new KodeValue(format(dateToFormat, args[0].text, simpleTokens, multiTokens), call.source);

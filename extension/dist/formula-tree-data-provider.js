    ...
    getTreeItem(element) {
        let item = (() => {
            if (element instanceof formula_js_1.Formula) {
                ...
                return new vscode.TreeItem('unimplemented item', vscode.TreeItemCollapsibleState.Expanded);
            }
        })();
        if (this._sourceDocument && element instanceof base_js_1.Evaluable) {
            item.command = {
                title: 'Highlight in editor',
                command: 'kodeine.highlightRange',
                arguments: [
                    this._sourceDocument,
                    element.source?.getStartIndex(),
                    element.source?.getEndIndex()
                ]
            };
        }
        return item;

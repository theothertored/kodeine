...
abstract class BaseToken implements IFormulaToken {
    ...
    protected readonly _endIndex: number;
...
class OperatorToken extends BaseToken {
    ...
    is(operatorText: string): boolean {
        return this._text === operatorText;
    }
}

class FunctionCallToken implements IFormulaToken {
...
class KodeineLexer implements ILexer {
    ...
    constructor(charReader: ICharReader) {
...
class PlainTextPart implements IFormulaPart {
    ...
    getOutput(): string {
        let output = '';
        for (var token of this.tokens) output += token.getStringRepresentation();
        return output;
    }
}
...
class KodeineParser implements IParser {
    ...
    private _parseNextPart(): IFormulaPart {
